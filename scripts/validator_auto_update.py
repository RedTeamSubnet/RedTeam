...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("validator_autoupdate.log"), logging.StreamHandler()],
)
...
def has_package_changes(remote_commit):
    ...
    try:
        ...
        diff_output = subprocess.getoutput(
            f"git diff HEAD..{remote_commit} --name-only"
        )
    ...
    while True:
        try:
            ...
            if should_update_local(local_commit, remote_commit):
                ...
                if status == "errored":
                    logging.warning(
                        f"{validator_process_name} is in error state, attempting another restart..."
                    )
                    restart_process(validator_process_name)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Auto-updater for RedTeam Subnet validator"
    )
    parser.add_argument(
        "--validator-process-name",
        default="validator_snxxx",
        help="PM2 process name of the validator",
    )
    parser.add_argument(
        "--process-name",
        default="validator_autoupdate",
