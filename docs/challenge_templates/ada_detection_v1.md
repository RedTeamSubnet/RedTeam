...
1. **Navigate to the AB Sniffer v5 Commit Directory**
    ...
    ```bash
    cd redteam_core/miner/commits/ada_detection_v1
    ```
...
2. **Build the Docker Image**
    ...
    ```bash
    docker build -t my_hub/ada_detection_v1-miner:0.0.1 .

    # For MacOS (Apple Silicon) to build AMD64:
    DOCKER_BUILDKIT=1 docker build --platform linux/amd64 -t myhub/ada_detection_v1-miner:0.0.1 .
    ```
...
4. **Push the Docker Image**
    ...
    ```bash
    docker push myhub/ada_detection_v1:0.0.1
    ```
...
5. **Retrieve the SHA256 Digest**
    ...
    ```bash
    docker inspect --format='{{index .RepoDigests 0}}' myhub/ada_detection_v1:0.0.1
    ```
...
6. **Update active_commit.yaml**
    ...
    ```yaml
    - ada_detection_v1---myhub/ada_detection_v1@<sha256:digest>
    ```
