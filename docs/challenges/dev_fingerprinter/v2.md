...
Each device will be tested in multiple "batches" across these browsers to ensure consistency.
...
### 2. `postProcess(fingerprint)`

This function handles reporting the fingerprint back to the server and cleaning up the environment.

- **Arguments**: `fingerprint` (the string returned by `runFingerprinting`)
- **Return Type**: `Promise<void>`
- **Goal**: Use `window.ENDPOINT` and `window.ORDER_ID` to POST the data, then clean up any global variables or logs.

---

## Example Implementation (Templates)

You can find the full template in the repository, but here is the required structure:

```javascript
// 1. Generation Logic
export async function runFingerprinting() {
    // Wait for DOM if needed
    // Collect attributes (Canvas, WebGL, Audio, etc.)
    // Create a hash
    return "generated_fingerprint_here";
}

// 2. Reporting & Cleanup Logic
