...
from cfg_analyser import CFGAnalyser


class Controller(BaseController):
    """
    Custom Controller that manages miner submissions, script similarity checks,
    and scoring adjustments before sending results to the Comparer.
    """
    ...
    def __init__(
        ...
        """
        Initializes the Controller with miner details and challenge configurations.

        Args:
            challenge_info (dict): Metadata about the challenge.
    ...
    def start_challenge(self):
        """
        Main function to process miner submissions. Runs miners in Docker, extracts outputs,
            ...
            try:
                ...
                if uid != self.baseline_commit.miner_uid:
                    miner_commit.scoring_logs.append(
                        ScoringLog(
                            miner_input=None, miner_output=None, score=0, error=str(e)
                        )
                    )
    ...
    def _score_miner_with_new_inputs(self, miner_commit, challenge_inputs):
        """
        Runs miners with challenge inputs, performs script similarity analysis,
        adjusts scores accordingly, and logs similarity for the Comparer.
        """
        for i, miner_input in enumerate(challenge_inputs):
            # 1. Get miner output from the /solve endpoint
            miner_output, error_message = self._submit_challenge_to_miner(miner_input)
            if miner_output is None:
                score = 0.0
                similarity_score = None  # No script to compare
            else:
                # 2. Compare miner_output against top miner scripts using CFGAnalyser
                similarity_score = self._calculate_script_similarity(miner_output)

                # 3. Apply similarity-based scoring adjustments
                if similarity_score >= 0.5:
                    score = 0.0  # Highly similar → No score
                else:
                    score = self._score_challenge(
                        miner_input=miner_input, miner_output=miner_output, task_id=i
                    )
                    score *= 1 - similarity_score  # Scale based on similarity

            # 4. Store similarity score in comparison logs for the Comparer
            self._log_comparison_score(miner_commit, similarity_score)

            # 5. Store results in scoring logs
            log = ScoringLog(
    ...
    def _calculate_script_similarity(self, miner_output):
        """
        ...
        reference_scripts = [
            ...
            if commit.miner_output
        ]
        similarity_scores = [
            CFGAnalyser.compare(miner_output, ref_script)
            for ref_script in reference_scripts
        ]

        return (
            max(similarity_scores) if similarity_scores else 0.0
        )  # Highest similarity score

    def _log_comparison_score(self, miner_commit, similarity_score):
        """
        Stores similarity score in comparison logs for later penalty checks in Comparer.
        """
        if similarity_score is not None:
            miner_commit.comparison_logs["script_similarity"] = similarity_score
