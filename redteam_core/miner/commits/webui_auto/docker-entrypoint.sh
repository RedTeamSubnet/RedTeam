...
_doStart()
{
	# exec python -u ./app.py || exit 2
	exec uvicorn app:app --host=0.0.0.0 --port="${WUM_API_PORT:-10002}" --no-access-log --no-server-header --proxy-headers --forwarded-allow-ips='*' || exit 2
	exit 0
}
main()
{
	...
	chmod ug+x "${WUM_API_DIR}/app.py" || exit 2
